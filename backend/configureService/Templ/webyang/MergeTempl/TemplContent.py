...
def CheckCon(f,rootdir,filename):
    f.save(rootdir + filename)
    file_object = open(rootdir + filename)
    try:
        all_the_text = file_object.read()  # 读取文件内容
        all_the_text = re.sub(r"\n[\s| ]*\n","",all_the_text)
    finally:
        file_object.close()
    TemplnameList, OriginContentDict, OutputJsonDict = SplitCon(all_the_text,filename)
    if TemplnameList==False:
        os.remove(rootdir + filename)
    return TemplnameList, OriginContentDict, OutputJsonDict


def SplitCon(all_the_text,filename):#用于上传莫本文件的预处理
    TemplnameList = []
    OriginContentDict = {}
    OutputJsonDict = {}
    cc = True
    ...
    for i in range(len(TemplList)):
        ...
        if JsonSingleCon != [] and JsonSingleCon != None and isinstance(JsonSingleCon, str) == False:
            ...
            cc = False
            return cc,filename, JsonSingleCon
    if cc:
        return TemplnameList, OriginContentDict, OutputJsonDict

def ReadFile(rootdir):#用于读与库中已有模板文件的数据预处理
    TemplnameList = []
    OriginContentDict = {}
    OutputJsonDict = {}
