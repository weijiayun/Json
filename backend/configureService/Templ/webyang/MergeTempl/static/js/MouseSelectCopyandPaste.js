...
function ResetBackgroundColor(obj,tbtype) {
    ...
    for(var i1=tbrowstart;i1<tbrowsend;i1++){
            backgroundcolor = RowsColor(i1+1);
...
function OnMouseDown(field,tbtype) {
    ...
    ResetBackgroundColor(field,tbtype);
    if(MouseSelect.this){
        tbtype = MouseSelect.pretableType;
        ResetBackgroundColor(MouseSelect.this,tbtype);
        }
...
function onMouseOver() {
    ...
    ResetBackgroundColor(this,tbtype);
    for(var i=0;i<height;i++){
        for(var j=0;j<width;j++){
...
function CopyAndPaste() {
    $("body").bind({
        copy: function (e) {//copy事件
            ...
            clipboardData.setData('Text', csv.replace(/<br>/g,""));
            return false;//否则设不生效
