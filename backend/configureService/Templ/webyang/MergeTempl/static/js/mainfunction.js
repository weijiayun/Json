...
//存入原始模板,后期编号，，，
var JSONDICT,jsonREF,jsonDict,jsonList;
...
window.onload = function () {
    JSONDICT = document.getElementById("JsonDict").innerHTML;
    jsonREF = JSON.parse(JSONDICT)["REFLIST"];//字符串解析出REFLIST
    jsonDict = JSON.parse(JSONDICT)["REFERENCES"];//字符串解析出content
    jsonList = JSON.parse(JSONDICT)["JSONLIST"];//字符串解析出json
    var html = "";
    $("#StrategySelect").empty();
    for (var e = 0; e < jsonREF.length; e++)
        html += "<option value='{0}'>{0}</option>".format(jsonREF[e]);
...
function Option_click(obj){
    ...
    var context = $.trim(document.getElementById(refid).innerHTML).replace(/\s+/g,"|");
    ...
    if (valtemp == "Fixed"){
        ...
        for (i=0 ;i< TEMPLATES.length;i++){
            ...
            if((temp["Category"]==conref)||(temp["BaseName"]==conref)||(conref==TEMPLATES[i])){
                isIn=true;
    ...
    if (CHECKLIST.length == 0)
        ...
        for (i = 0; i < CHECKLIST.length; i++) {
            ...
            if(result!=[] && result[0]){
                ...
                else if((sub['Operate']!="")&& (sub['Field']!="")&& (sub['Value']!="")&& (sub['Source']!="")){
                    document.getElementById(CHECKLIST[i].id).checked = false;
                    html += "<div id='Option_{0}'>{0} <input type='button' value='Delete' style='margin-bottom: 4px;' onclick='DeleteOption(this);'>".format(CountOp);
                    html += "<div class='table-responsive'><table class='table table-bordered table-hover'>";
                    html += "<tbody><tr><td style='font-weight:bold;'>Operate</td><td>{0}</td></tr>".format(sub['Operate']);
...
function FIXED(judge,type,field_obj,text){
    ...
    if (field_obj[0].firstChild.id.indexOf("input") != -1) { //input
        if (field_obj[0].firstChild.value == ''){
            ...
            var is = /^((-\d+)|(0+))$/;　　//非正整数
            var dou = /^(-?\d+)(\.\d+)?$/;//浮点数
            var vall = field_obj[0].firstChild.value;
            var inputvalue = Number(vall);
            if ((isNaN(inputvalue)) && ((type=='uint32_t')||(type=='uint32')||(type=="uint16")
                    ||(type=="int32_t")||(type=="sint32")||(type=="double")||(type=="uint16_t"))){//输入为字符串
                alert("字段类型不一致:输入为字符串，字段里有Number类型!");
                judge = false;
            }
            else if (isNaN(inputvalue) == false) { //输入为数字
                if ((s.test(vall)) && (type=='double')) {//判断整型
                    alert("字段类型不一致：输入为整数，字段里有double类型!");
                    judge = false;
                }
                else if ((s.test(vall)) && (type=='string')) {
                    alert("字段类型不一致：输入为整数，字段里有string类型!");
                    judge = false;
                }
                else if (is.test(vall) && ((type=='uint32_t')||(type=='uint32')||(type=="uint16")||(type=="uint16_t"))) {//非正整数
                    alert("字段类型不一致：输入为非正整数，字段里有正整数类型!");
                    judge = false;
                }
                else if (dou.test(vall) && ((type=='uint32_t')||(type=='uint32')||(type=="uint16")
                    ||(type=="int32_t")||(type=="sint32")||(type=="string")||(type=="uint16_t"))) {//浮点数
...
function MERGE(judge,Merge,Mergedict){
    ...
    if (type[1]=="bool" && (typelist.indexOf('uint32_t')||typelist.indexOf('uint32')||typelist.indexOf('string')||typelist.indexOf('uint16_t')
    ...
    else if (type[1]=="string" && (typelist.indexOf('uint32_t')||typelist.indexOf('uint32')||typelist.indexOf('bool')||typelist.indexOf('uint16_t')
        ||typelist.indexOf('int32_t')||typelist.indexOf('sint32')||typelist.indexOf('double')||typelist.indexOf('uint16'))){
        alert(" 该字段的string类型不能和其他字段进行Merge!");
        judge = false;
    }
    else if (type[1]=="double" && (typelist.indexOf('uint32_t')||typelist.indexOf('uint32')||typelist.indexOf('bool')||typelist.indexOf('uint16_t')
    ...
    else if (type[1]=="uint16" && (typelist.indexOf('double')||typelist.indexOf('uint32')||typelist.indexOf('bool')||typelist.indexOf('uint16_t')
        ...
        if (typelist.indexOf('double') || typelist.indexOf('uint32_t') || typelist.indexOf('bool')||typelist.indexOf('uint16_t')
        ...
        else if (typelist.indexOf("uint16_t"))
            text.replace(/int32_t/, "uint16_t");
    }
    op['Operate'] = 'Merge';
    op['Field'] = text;
...
function Finish(){
    if ($("#Templnameinp").val()=="")
        alert("未输入合并后模板的名称！");
    else if($("#Templversioninp").val()=="")
        alert("未输入合并后模板的版本号！");
    else if(DictLength(OPTIONLIST)==0)
        alert("未进行任何合并操作！");
    else {
        var Templname = $("#Templnameinp").val();
        var Templversion=$("#Templversioninp").val();
        OPTIONLIST["NewName"]=Templname;
        OPTIONLIST["NewVersion"]=Templversion;
...
function UpdateSel(ind) {
    jsonREF = ind["REFLIST"];//字符串解析出REFLIST
    var html = "";
    $("#StrategySelect").empty();
    for (var e = 0; e < jsonREF.length; e++)
        html += "<option value='{0}'>{0}</option>".format(jsonREF[e]);
...
$(function () {
    $('#fileupload').fileupload({
        dataType: 'json',
            ...
            var ind = data.result;
            if ("FILENAME" in ind)
                alert("filename:"+data.result.FILENAME+"\nerrorname:"+data.result.ERROR);
            else if ("REFLIST" in ind){
                jsonREF = ind["REFLIST"];//字符串解析出REFLIST
                jsonDict = ind["REFERENCES"];//字符串解析出content
                jsonList = ind["JSONLIST"];//字符串解析出json
                UpdateSel(ind);
