...
class MessagePlugin(object):
    def __init__(self, mesgHandle):
        ...
        self.createGeneric = mesgHandle.createGeneric
...
class MessageHandler(object):
    ...
    def send(self,targretId, proto,spec,body,requestId=0):
        return self.view.send(targretId, proto, spec, body, requestId)


    def load(self, conf):
        for plugin in self.plugins:
            if plugin.name in conf:
                plugin.load(conf[plugin.name])
...
class Factory(object):
    ...
    @staticmethod
    def createMessageHandles(patterns):
        mesgHandle = MessageHandler()
