...
function HtmlExcelAll(TemplatesUnitIdPrefix) {
    ...
    function expandMatrix(instance, td, row, col, prop, value, cellProperties) {
        ...
        if($(td).parent().index() != currentRowindex){
            if($(td).children().length==0) {
                $(td).html("<button onclick='MatButton(this,event)'>{0}</button>".format(prop));
    ...
    function expandList(instance, td, row, col, prop, value, cellProperties) {
        ...
        if($(td).parent().index() != currentRowindex){
            if($(td).children().length==0) {
                $(td).html("<button  onclick='ListButton(this,event)'>{0}</button>".format(prop));
    ...
    for (var varName in FieldsVar) {
        ...
        if (!FieldsVarAttrs.Reference) {
            if (FieldsVarAttrs.Type == "sint_32" || FieldsVarAttrs.Type == "uint_32" || FieldsVarAttrs.Type == "string")
                NumberandStringTemplate(structName, FieldsVarAttrs, TemplateUnitIdPrefix, tempdict,modalId);
            else if (FieldsVarAttrs.Type == "enum") {
                var enumList = Object.getOwnPropertyNames(StrategyDict[FieldsVarAttrs.EleType].Fields);
                enumTemplate(structName, enumList, FieldsVarAttrs, TemplateUnitIdPrefix, tempdict,modalId)
            }
            else if (FieldsVarAttrs.Type.match("mat") || FieldsVarAttrs.Type.match("vec"))
                matrixTemplate(structName, FieldsVarAttrs, TemplateUnitIdPrefix, tempdict,modalId);
            else if (FieldsVarAttrs.Type == "bool")
                boolTemplate(structName, FieldsVarAttrs, TemplateUnitIdPrefix, tempdict,modalId);
            else if (FieldsVarAttrs.Type.match("list"))
                listTamplate(structName, StrategyDict[FieldsVarAttrs.EleType].Fields, FieldsVarAttrs, false, structName, TemplateUnitIdPrefix, tempdict,modalId);
        }
        else {
            if (FieldsVarAttrs.Type.match("list"))
                ...
                var refFeedback = $.ajax("/reference/{0}".format(reference), {
                    ...
                    enumTemplate(structName,data,FieldsVarAttrs, TemplateUnitIdPrefix, tempdict,modalId);
                }).fail(function (xhr,status) {
                    alert("Failed: {0}\n Reason: {1}\n".format(xhr.status,status));
                });
            }
...
function boolTemplate(structname,VarAttrs,TemplateUnitIdPrefix,valueDict,modalId) {
    ...
    $("#{0}".format(modalId)).children().eq(0).append(boolhtml)

}

function enumTemplate(structname,enumlist,VarAttrs,TemplatesUnitIdPrefix,valueDict,modalId) {
    var enumhtml = "";
    ...
    enumhtml += "</ul></span></li>";
    $("#{0}".format(modalId)).children().eq(0).append(enumhtml);
}
...
function NumberandStringTemplate(structname,VarAttrs,TemplatesUnitIdPrefix,valueDict,modalId) {
    var NumStrhtml = "",temp;
    NumStrhtml += '<li style="display: block" >';
     if(VarAttrs.Requiredness )
         temp ='<span class="glyphicon glyphicon-asterisk asterisk"></span>';
    else
         temp ='<span></span>';
    ...
    if(VarAttrs.Default == null)
        VarAttrs.Default = "";
    NumStrhtml +='<td contenteditable="true" spellcheck="false" class="jsoneditor-number jsoneditor-value jsoneditor-listinput" onblur="inputTypeChecktips(this)" data-type="\'{4}\'" id="{0}{1}{2}">{3}</td>'.format(TemplatesUnitIdPrefix,structname,VarAttrs.Name,VarAttrs.Default,VarAttrs.Type);
    NumStrhtml += '</tr></table></li>';
    $("#{0}".format(modalId)).children().eq(0).append(NumStrhtml)
}
function listTamplate(structname,listTypeFieldsDict,VarAttrs,IsReference,preStructName,TemplatesUnitIdPrefix,valueDict,modalId) {
    var struct = "";
    var name = "";
    var listIdBase = "";
    if(IsReference){
    ...
    $("#{0}".format(modalId)).children().eq(0).append(listhtml);
...
function listRefTemplate(structname,VarAttrs,TemplatesUnitIdPrefix,valueDict,modalId) {
    ...
    $("#{0}".format(modalId)).children().eq(0).append(listrefhtml);
...
function matrixTemplate(structname,VarAttrs,TemplatesUnitIdPrefix,valueDict,modalId) {
    ...
    $("#{0}".format(modalId)).children().eq(0).append(mathtml);
