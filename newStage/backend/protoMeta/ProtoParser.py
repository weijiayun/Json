...
class EnumState(StateMachine.StateBase):
    ...
    def do(self,evt):
        ...
        if valueMatch is None:
            ...
            syntaxError.msg = 'encounter syntax error when parse enum %s ,line %s' % (self.spec.getName(),evt.line)
            raise syntaxError
...
class StructState(StateMachine.StateBase):
    ...
    def entry(self,evt):
        ...
        self.alias = self.spec.getName().lower()
    ...
    def do(self,evt):
        ...
        if fieldMatch is None:
            syntaxError = exceptions.SyntaxError()
            syntaxError.msg = 'encounter syntax error when parse struct %s ,line %s' % (self.spec.getName(),evt.line)
            raise syntaxError
        else:
...
class TypedefState(StateMachine.StateBase):
    ...
    def entry(self,evt):
        ...
        TypedefState.spec = fromSpec
        TypedefState.alias = self.typedefMatch.group(2).lower()
...
class MessageState(StateMachine.StateMachine):
    ...
    def __onStateChanged(self,fromState,toState):
        isFromAlias = TypedefState.spec is not None

        state = fromState if not isFromAlias else TypedefState
        if isinstance(state,StructState) or (isFromAlias and isinstance(state.spec, TypeSpec.StructSpec)):
            #print state.alias,self.spec.getName()
            if state.alias == self.spec.getName():
                if self.spec.getInitiatorSpec() is None:
                    self.spec.setInitiatorSpec(state.spec)
                else:
                    raise TooManyInitiator(self.spec.getName())
            elif state.alias.endswith('response'):
                if self.spec.getAcceptorSpec() is None:
                    self.spec.setAcceptorSpec(state.spec)
                else:
    ...
    def isMessageCode(cls,evt):
        ...
        if cls.messageCodeMatch is not None:
            columns = evt.line.split('_')
            return columns[1] != 'ERROR' and columns[1] != 'GROUP'
        return False
...
class ProtoParser(object):
    ...
    def parse(self,path,specDict,excludes = None):
        ...
        if exceptions is not None:
            modules = [m for m in modules if m not in excludes]
    ...
    def __parseModule(self,path,module):
        ...
        if self.mainStateMachine.getCurrentState() != self.initState or self.messageState.getCurrentState() != self.initState:
            syntaxError = exceptions.SyntaxError()
            syntaxError.msg = 'uncompleted module' + module
            raise syntaxError
